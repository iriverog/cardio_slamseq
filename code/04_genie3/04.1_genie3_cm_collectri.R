...
for(seed in seeds){
  set.seed(seed)
  print(paste0("Running GENIE3 iteration ", iteration, "/100 (", Sys.time(), ")"))
  weightMat <- GENIE3(exprMatrix = exprMatFil, nCores = 4, regulators = regulatorsList, targets = names(regulatorsList), returnMatrix = TRUE)
  if(iteration == 1){
...
saveRDS(avgLinkWeight, "results/04_genie3/genie3_avg_weight_cm_net.rds")
saveRDS(linkList, "results/04_genie3/genie3_real_weights_cm_net.rds")
